...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <div className="card-accent group overflow-hidden flex flex-col bg-white">
      {/* Card header */}
      <div className="p-5 pb-0">
        <div className="flex items-start justify-between mb-3">
          {/* Icon */}
          <div className="w-12 h-12 rounded-[var(--radius-md)] bg-[var(--color-apex-50)] border border-[var(--color-apex-100)] flex items-center justify-center text-[var(--color-apex-600)] group-hover:bg-[var(--color-apex-100)] transition-colors">
            <Download size={22} />
          </div>
        ...
        {/* Title */}
        <h3 className="font-bold text-[var(--color-text-primary)] text-base leading-snug mb-1.5 group-hover:text-[var(--color-apex-600)] transition-colors line-clamp-2 tracking-tight">
          {product.name}
        </h3>
          ...
          <ul className="space-y-1">
            {product.features.slice(0, 2).map((feature, i) => (
              <li key={i} className="flex items-center gap-2 text-xs text-[var(--color-text-muted)]">
                <CheckCircle size={12} className="text-[var(--color-apex-500)] shrink-0" />
                {feature}
              </li>
            ))}
          </ul>
