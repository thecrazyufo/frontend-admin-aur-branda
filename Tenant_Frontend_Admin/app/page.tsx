...
export default function PortalPage() {
  ...
  return (
    <div className="relative min-h-screen bg-zinc-950 flex items-center justify-center py-16 px-4 sm:px-6 lg:px-8 font-sans overflow-hidden">
      {/* Background Glow */}
      ...
      <div className="w-full max-w-5xl z-10 space-y-12">
        ...
        <header className="text-center space-y-4">
          ...
          <h1 className="text-3xl font-extrabold tracking-tight sm:text-4xl text-zinc-50">
          ...
          <div>
            <Link href="/admin/login">
              <Button variant="outline" className="text-xs font-semibold py-1.5 px-4 h-8 gap-2 bg-zinc-900 border-zinc-800 text-zinc-300 hover:text-zinc-50">
                🔑 Super Admin login
              </Button>
            </Link>
          </div>
          ...
          {sites.map((site) => {
            ...
            return (
              <Card
                ...
                className={`group flex flex-col justify-between p-5 border cursor-pointer text-left transition-all duration-200 hover:-translate-y-1 hover:shadow-lg dark:bg-zinc-900/20 backdrop-blur-md ${accentColors[colorKey] || accentColors["indigo"]}`}
              >
                <div>
                  ...
                  <h3 className="text-sm font-bold text-zinc-200 group-hover:text-zinc-50">{site.name}</h3>
        ...
        {selectedBrand && (
          ...
          >
            <Card 
              className="w-full max-w-md bg-zinc-900 border-zinc-850 p-6 space-y-6"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-start justify-between">
                <div>
                  <CardTitle className="text-base font-extrabold text-zinc-50">
                ...
                <button 
                  className="text-zinc-500 hover:text-zinc-350 text-xl font-medium leading-none focus:outline-none"
                  onClick={() => setSelectedBrand(null)}
                >
                  &times;
                </button>
              ...
              <div className="space-y-2.5">
                {ROLES.map((role) => (
                  <button
                    key={role.id}
                    className="w-full flex items-center gap-4 p-4 rounded-xl border border-zinc-800 bg-zinc-950/40 text-left hover:bg-indigo-500/5 hover:border-indigo-500/30 transition-all duration-200 group"
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-bold text-zinc-200 group-hover:text-zinc-50">
                        {role.name}
                      </div>
