...
function HelpContent() {
  ...
  const activeTab = "help" as string;
  ...
  const [helpModal, _setHelpModal] = useState<any | null>(null);
  
  const [isDirty, setIsDirty] = useState(false);
  const prevModalRef = useRef<string | null>(null);

  useEffect(() => {
    const currentStr = helpModal ? JSON.stringify(helpModal) : null;
  ...
  async function saveHelpArticle(isDraftOnly: boolean) {
    ...
    try {
      ...
      if (isDraftOnly) {
        ...
        setIsDirty(false); setHelpModal(null);
      }
      loadData();
    } catch {
                        ...
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center flex-wrap gap-2">
                            ...
                            <div className="flex flex-wrap gap-1">
                              {h.productNames && h.productNames.map((name: string) => (
