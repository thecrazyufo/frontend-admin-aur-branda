...
`}} />
 <div className="admin-shell light flex min-h-screen bg-background text-foreground font-sans transition-colors duration-200">
 {/* ── Sidebar ── */}
 <aside className={`sticky top-0 h-screen flex flex-col border-r border-border bg-card transition-all duration-200 ease-out overflow-hidden z-20 ${sidebarOpen ?"w-60" :"w-14"}`}>
 <div className="flex items-center justify-between px-3.5 h-14 border-b border-border shrink-0">
 ...
 {sidebarOpen && <span className="font-bold text-xs tracking-tight text-foreground truncate max-w-[120px]">{activeBrandConfig?.name ||"Admin Portal"}</span>}
 </Link>
 <button
 ...
 <label className="block text-[8px] font-bold text-muted-foreground uppercase tracking-wider">
 Active Storefront {isSuperAdmin(userRole) ?"🔓" :"🔒"}
 </label>
 {isSuperAdmin(userRole) ? (
 <select
 className="w-full bg-muted/50 border border-border text-foreground text-xs rounded-md px-2 py-1.5 outline-none focus:ring-1 focus:ring-indigo-500 transition-shadow appearance-none cursor-pointer"
 value={siteId}
 ...
 <div className="w-full bg-muted border border-border text-foreground text-xs rounded-md px-2 py-1.5 font-medium flex items-center gap-2 cursor-not-allowed opacity-90">
 ...
 <div className="w-full bg-indigo-50/40 border border-indigo-100 rounded-md py-1 text-center text-[11px] font-semibold text-indigo-600">
 Global Context 🌍
 </div>
 </div>
 )}
 ...
 <div
 className={`flex items-center justify-between px-2.5 py-1.5 rounded-md text-xs font-medium cursor-pointer transition-colors relative ${isActive ?"text-foreground bg-muted font-semibold" :"text-muted-foreground hover:text-foreground hover:bg-muted/50"}`}
 ...
 key={idx}
 href={item.href}
 className={`flex items-center gap-2.5 px-2.5 py-1.5 rounded-md text-xs font-medium transition-colors relative ${isActive ?"text-foreground bg-muted font-semibold" :"text-muted-foreground hover:text-foreground hover:bg-muted/50"}`}
 >
 <span className="shrink-0 opacity-80">{item.icon}</span>
 {sidebarOpen && <span>{item.label}</span>}
 </Link>
 ...
 <div className="p-2 border-t border-border flex flex-col gap-1.5 shrink-0 bg-card">
 ...
 <span className="text-[11px] font-semibold text-foreground truncate">{username}</span>
 ...
 <button
 className="flex items-center justify-center gap-1.5 w-full px-2 py-1 text-[11px] font-semibold rounded-md border border-border text-muted-foreground hover:text-red-600 hover:border-red-200 hover:bg-red-50 transition-all cursor-pointer"
 ...
 {/* Topbar / Header */}
 <header className="flex items-center justify-between px-6 h-14 border-b border-border bg-card/80 backdrop-blur-md sticky top-0 z-10 shrink-0">
 <div className="flex items-center gap-5 min-w-0">
 <h1 className="text-xs font-bold text-foreground uppercase tracking-wider truncate">
 ...
 className="w-full bg-background border border-border rounded-md pl-7 pr-3 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500/20 transition-all placeholder:text-muted-foreground"
 />
 </div>
 </div>
 ...
 {(pathname.includes("/seo") || pathname.includes("/cc") || pathname.includes("/admin") || pathname.includes("/brand") || pathname ===`/${siteId}`) && (
 <div className="flex items-center gap-0.5 bg-muted p-0.5 rounded-md border border-border shrink-0">
 {[
 { id:"brandA", label:"A" },
 { id:"brandB", label:"B" },
 { id:"brandC", label:"C" },
 ...
 className={`w-6 h-6 flex items-center justify-center text-[10px] font-bold rounded transition-all ${isActive ?"bg-card shadow-sm text-indigo-600" :"text-muted-foreground hover:text-foreground"} ${isAllowed ?"cursor-pointer" :"opacity-30 cursor-not-allowed"}`}
 ...
 {/* Notification bell */}
 <button className="p-1.5 rounded-md border border-border bg-card hover:bg-muted/50 text-zinc-500 hover:text-foreground transition-all cursor-pointer relative" title="Notifications">
 <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
 ...
 {/* Theme toggle */}
 <button
 className="p-1.5 rounded-md border border-border bg-card hover:bg-muted/50 text-zinc-500 hover:text-foreground transition-all cursor-pointer"
 onClick={toggleTheme}
 ...
 target="_blank"
 rel="noreferrer"
 className="flex items-center gap-1 px-2.5 py-1.5 rounded-md border border-border bg-card hover:bg-muted/50 text-xs font-semibold text-muted-foreground hover:text-foreground transition-all decoration-none"
