...
import { Menu, X, ChevronDown, Search, ArrowRight, Phone, Smile } from "lucide-react";
...
export default function Navbar({
  ...
  return (
    <>
      ...
      {phone && (
        <div className="bg-[var(--color-uncle-900)] text-[var(--color-uncle-200)] text-xs py-2 hidden md:block">
          <div className="container-custom flex items-center justify-between">
            ...
            <div className="flex items-center gap-6">
              ...
              <span className="text-[var(--color-uncle-400)]">•</span>
              <span>30-Day Money-Back Guarantee</span>
              <span className="text-[var(--color-uncle-400)]">•</span>
              <a
                href="/download"
                className="text-[var(--color-accent-400)] hover:text-[var(--color-accent-300)] font-semibold transition-colors"
              >
                Free Trial ↗
              </a>
            </div>
      ...
      <nav
        className={`sticky top-0 z-50 bg-white transition-all duration-200 ${
          scrolled
            ? "shadow-[0_4px_20px_rgb(16_185_129/0.05)] border-b border-[var(--color-border)]"
                  ...
                  <a
                    ...
                    className={`flex items-center gap-1 px-4 py-2.5 rounded-[var(--radius-md)] text-[14.5px] font-semibold transition-all tracking-tight ${
                      activeDropdown === item.label
                        ? "text-[var(--color-uncle-700)] bg-[var(--color-uncle-50)] shadow-sm"
                        : "text-[var(--color-text-secondary)] hover:text-[var(--color-uncle-600)] hover:bg-[var(--color-surface-2)] hover:shadow-sm"
                    }`}
                  >
                    {item.label}
                    {item.children && (
