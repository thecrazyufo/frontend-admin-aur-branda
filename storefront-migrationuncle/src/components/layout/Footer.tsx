import { Mail, Phone, MapPin, Shield, CheckCircle, Award, Smile } from "lucide-react";
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-10">
          ...
          <div className="lg:col-span-2">
            <a href="/" className="flex items-center gap-2.5 mb-5 group w-fit">
              <div className="w-9 h-9 rounded-full bg-[var(--color-uncle-500)] flex items-center justify-center shadow-sm group-hover:shadow-md group-hover:-translate-y-0.5 transition-all">
                <Smile size={20} className="text-white" strokeWidth={2.5} />
              </div>
              <div className="leading-none">
                <span className="font-extrabold text-[var(--color-text-primary)] text-[20px] tracking-tight" style={{ fontFamily: "var(--font-serif)" }}>
                  Migration
                </span>
                <span className="font-bold text-[var(--color-uncle-600)] text-[20px] tracking-tight" style={{ fontFamily: "var(--font-serif)" }}>
                  &nbsp;Uncle
                </span>
              </div>
            </a>
            ...
            <p className="text-sm text-[var(--color-text-muted)] leading-relaxed mb-6 max-w-xs">
              {description ||
                "Your friendly, reliable partner for moving emails, files, and data safely without the headache."}
            </p>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {trustBadges.map((badge) => (
                ...
                >
                  <span className="text-[var(--color-uncle-600)]">{badge.icon}</span>
                  {badge.text}
                </div>
              ))}
            </div>
              ...
              <div className="flex items-center gap-2">
                {socialList.map(({ href, label, abbr }) => (
                  <a
                    ...
                    aria-label={label}
                    className="w-8 h-8 rounded-[var(--radius-md)] bg-white border border-[var(--color-border)] flex items-center justify-center text-[var(--color-text-muted)] hover:text-[var(--color-uncle-600)] hover:border-[var(--color-uncle-300)] hover:bg-[var(--color-uncle-50)] transition-all text-xs font-bold shadow-xs"
                  >
                    {abbr}
                  </a>
                ))}
              ...
              <ul className="space-y-2.5">
                {links.productLinks.map((item) => (
                  <li key={item.href}>
                    <a
                      href={item.href}
                      className="text-sm text-[var(--color-text-muted)] hover:text-[var(--color-uncle-600)] transition-colors font-medium"
                    >
                      {item.label}
                    </a>
                  </li>
                ...
                <li>
                  <a
                    href="/products"
                    className="text-sm text-[var(--color-uncle-600)] hover:text-[var(--color-uncle-700)] font-semibold"
                  >
                    View All →
                  </a>
                </li>
            ...
            <ul className="space-y-3">
              {phone && (
                <li>
                  <a
                    href={`tel:${phone}`}
                    className="flex items-start gap-2.5 text-sm text-[var(--color-text-muted)] hover:text-[var(--color-uncle-600)] transition-colors font-medium"
                  >
                    <Phone size={14} className="mt-0.5 shrink-0 text-[var(--color-uncle-500)]" />
                    {phone}
                  </a>
                </li>
              )}
              {email && (
                <li>
                  <a
                    ...
                    className="flex items-start gap-2.5 text-sm text-[var(--color-text-muted)] hover:text-[var(--color-uncle-600)] transition-colors font-medium"
                  >
                    <Mail size={14} className="mt-0.5 shrink-0 text-[var(--color-uncle-500)]" />
                    {email}
                  </a>
                </li>
              )}
              {address && (
                <li>
                  <div className="flex items-start gap-2.5 text-sm text-[var(--color-text-muted)]">
                    <MapPin size={14} className="mt-0.5 shrink-0 text-[var(--color-uncle-500)]" />
                    {address}
                  </div>
                </li>
              )}
      ...
      >
        <div className="container-custom py-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-5 flex-wrap justify-center">
            ...
            <a
              href={adminLoginUrl}
              className="text-xs text-[var(--color-text-faint)] hover:text-[var(--color-uncle-600)] font-semibold transition-colors"
            >
              🔒 Admin
            </a>
          </div>
