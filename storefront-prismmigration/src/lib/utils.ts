...
export function parseMarkdownToHtml(markdown: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^(-{3,}|\*{3,}|_{3,})$/.test(trimmed)) {
      closeListIfOpen();
      closeBlockquoteIfOpen();
      html += '<hr class="my-6 border-[#334155] border-t border-[#334155]" />';
      continue;
    }
    ...
    // Unordered list
      ...
      if (!inList || listType !== "ul") {
        closeListIfOpen();
        html += '<ul class="list-disc list-inside space-y-1.5 my-3 pl-4 text-[#E2E8F0]">';
        inList = true;
        listType = "ul";
      }
    ...
    // Ordered list
      ...
      if (!inList || listType !== "ol") {
        closeListIfOpen();
        html += '<ol class="list-decimal list-inside space-y-1.5 my-3 pl-4 text-[#E2E8F0]">';
        inList = true;
        listType = "ol";
      }
    ...
    // Paragraph
      ...
      closeListIfOpen();
      html += `<p class="my-3 text-[#E2E8F0] leading-relaxed">${processInline(trimmed)}</p>`;
    }
  }
