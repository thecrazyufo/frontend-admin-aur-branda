...
/** Assign a subtle color class per format type based on Refraction design system */
...
function MigrationFlowHeader({ sources, targets }: { sources: string[]; targets: string[] }) {
  ...
  return (
    <div className="w-full h-[130px] bg-[var(--ink-void)] border-b border-[var(--circuit-line)] relative overflow-hidden flex-shrink-0 flex items-center justify-between px-4 gap-2">
      {/* Subtle animated radial glow behind prism */}
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div
          className="w-32 h-32 rounded-full bg-[var(--signal-teal)] opacity-[0.05] blur-2xl animate-pulse"
      ...
      <div
        ...
        style={{
          backgroundImage: "radial-gradient(circle, var(--signal-teal) 1px, transparent 1px)",
      ...
      <div className="flex flex-col gap-1.5 z-10 min-w-0 flex-1">
        {hasFlow && sources.length > 0 ? (
          ...
          <>
            <span className="h-4 w-12 rounded bg-stone-850 animate-pulse border border-[var(--circuit-line)]" />
            <span className="h-4 w-10 rounded bg-stone-850 animate-pulse border border-[var(--circuit-line)]" />
          </>
        )}
      </div>
      ...
      <div className="flex items-center gap-1.5 z-10 flex-shrink-0">
        ...
        <div className="flex items-center gap-0.5 opacity-40">
          <div className="w-3 h-px bg-[var(--signal-teal)]" />
          <div className="w-1.5 h-px bg-[var(--signal-teal)]" />
          <div className="w-1 h-px bg-[var(--signal-teal)]" />
        </div>
        ...
        <div className="relative flex items-center justify-center">
          {/* Outer glow ring */}
          <div
            className="absolute w-14 h-14 rounded-full border border-[var(--signal-teal)]/20 animate-spin"
            style={{ animationDuration: "16s" }}
          />
          <svg
            viewBox="0 0 100 100"
            className="w-10 h-10 drop-shadow-[0_0_12px_rgba(0,200,160,0.5)]"
            fill="none"
          >
            {/* Gradient fill */}
            <defs>
              <linearGradient id="prismGrad" x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%" stopColor="var(--signal-teal)" stopOpacity="0.8" />
                <stop offset="100%" stopColor="#4F46E5" stopOpacity="0.6" />
              </linearGradient>
              <linearGradient id="prismStroke" x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%" stopColor="#00E8B8" />
            ...
            <line x1="50" y1="10" x2="50" y2="85" stroke="var(--signal-teal)" strokeWidth="1" strokeOpacity="0.4" />
            <line x1="50" y1="10" x2="30" y2="85" stroke="#818CF8" strokeWidth="0.8" strokeOpacity="0.3" />
            <line x1="50" y1="10" x2="70" y2="85" stroke="#00E8B8" strokeWidth="0.8" strokeOpacity="0.3" />
          </svg>
        </div>
      ...
      <div className="flex flex-col gap-1.5 z-10 min-w-0 flex-1 items-end">
        {hasFlow && targets.length > 0 ? (
          ...
          <>
            <span className="h-4 w-14 rounded bg-stone-850 animate-pulse border border-[var(--circuit-line)]" />
            <span className="h-4 w-10 rounded bg-stone-850 animate-pulse border border-[var(--circuit-line)]" />
          </>
        )}
      </div>
      ...
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-20 h-4 bg-gradient-to-t from-[var(--signal-teal)]/10 to-transparent blur-sm pointer-events-none" />
    </div>
  );
}
...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <div className="bg-[var(--slate-surface)] border border-[var(--circuit-line)] rounded-xl shadow-2xl transition-all duration-300 hover:border-[var(--signal-teal)]/40 hover:shadow-[0_0_30px_rgba(0,200,160,0.06)] group overflow-hidden relative flex flex-col h-full min-h-[380px]">

      {/* Top gradient accent line on hover */}
      <div className="absolute top-0 left-0 w-full h-[1.5px] bg-gradient-to-r from-[var(--signal-teal)] via-[var(--brand-primary)] to-[var(--amber-alert)] z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      ...
      {product.badge && (
        <div className="absolute top-3 right-3 z-30">
          <span className={`px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider rounded border backdrop-blur-md ${
            ...
            product.badge === "popular"    ? "bg-teal-500/10 text-[var(--signal-teal)] border-teal-500/20" :
      ...
      <div className="p-5 flex flex-col flex-grow relative z-10">
        ...
        <div className="flex items-center justify-between mb-3">
          <span className="text-[9px] tracking-wider uppercase font-bold text-[var(--signal-teal)] bg-[var(--signal-teal-dim)] px-2 py-0.5 rounded border border-[var(--signal-teal)]/20 font-mono">
        ...
        {/* Product name */}
        <h3 className="font-bold text-white text-[16px] leading-snug mb-2 group-hover:text-[var(--signal-teal)] transition-colors line-clamp-1 tracking-tight">
        ...
        {destinationBadges.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-3">
            ...
            {targets.length > 5 && (
              <span className="inline-flex items-center px-1.5 py-0.5 text-[8px] font-bold text-[#6B7280] border border-[var(--circuit-line)] rounded leading-none">
                +{targets.length - 5}
              </span>
            )}
          </div>
        ...
        <div className="flex items-center gap-1.5 mb-3">
          <div className="flex text-[var(--amber-alert)]">
            {Array.from({ length: 5 }).map((_, i) => (
              <Star key={i} size={10} fill={i < Math.floor(product.rating || 0) ? "currentColor" : "none"} strokeWidth={1.5} />
            ))}
          </div>
          <span className="text-[11px] text-[var(--type-muted)]">
            {product.rating || 0} <span className="text-[var(--type-faint)]">({(product.reviewCount || 0).toLocaleString()})</span>
          </span>
        </div>

        {/* Short description */}
        <p className="text-[12px] leading-relaxed line-clamp-2 mb-4" style={{ color: 'var(--type-muted)' }}>
            ...
            <ul className="space-y-1.5 text-[11px]">
              {product.features.slice(0, 2).map((feat: string) => (
                <li key={feat} className="flex items-center gap-2">
                  <svg className="w-3.5 h-3.5 shrink-0 text-[var(--signal-teal)]" viewBox="0 0 16 16" fill="none">
                    <circle cx="8" cy="8" r="7" stroke="currentColor" strokeWidth="1.5" strokeOpacity="0.35"/>
                    <path d="M5 8l2 2 4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                  <span className="truncate" style={{ color: 'var(--type-muted)' }}>{feat}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="border-t border-[var(--circuit-line)] pt-4 mt-auto flex items-center justify-between">
          <div>
            <div className="text-[8px] uppercase tracking-wider font-bold" style={{ color: 'var(--type-faint)' }}>Starting at</div>
            {lowestPrice ? (
              <div className="flex items-baseline gap-0.5">
                <span className="font-extrabold text-[var(--amber-alert)] text-[16px] tracking-tight font-mono">
                  {formatPrice(lowestPrice.price)}
                </span>
                <span className="text-[9px]" style={{ color: 'var(--type-faint)' }}>/{lowestPrice.period === "one-time" ? "lifetime" : lowestPrice.period}</span>
              </div>
            ) : (
              <span className="font-extrabold text-[var(--signal-teal)] text-[15px] tracking-tight">Free</span>
            )}
          </div>
          <div className="flex items-center gap-2">
            <a
              href={`/download?product=${product.slug}`}
              className="border border-[var(--circuit-line)] bg-[var(--ink-void)] text-[var(--type-muted)] hover:text-white hover:border-[var(--signal-teal)]/40 py-2 px-3.5 rounded-lg text-[11px] min-h-[34px] flex items-center justify-center font-bold transition-all duration-300"
            >
              Trial
            </a>
            <a
              href={`/checkout?product=${product.slug}&tier=Standard`}
              className="text-black py-2 px-3.5 rounded-lg text-[11px] min-h-[34px] flex items-center justify-center font-extrabold transition-all duration-300 shadow-[0_0_12px_rgba(0,200,160,0.2)] hover:shadow-[0_0_20px_rgba(0,200,160,0.4)]"
